...
import { 
  User, Settings, Share2, Flame, Star, Trophy, 
  MapPin, GraduationCap, Zap, Calculator, Moon, 
  ThumbsUp, Bug, CheckCircle2, Users, ArrowUp, 
  MessageSquare, BookOpen, ChevronRight, PenTool,
  Award, History
} from 'lucide-react';
...
const user = {
  ...
  badges: [
    ...
    { 
      ...
      icon: Moon, 
      color: "text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-400/10",
    ...
    { 
      ...
      icon: ThumbsUp, 
      color: "text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-400/10",
  ...
  groups: [
    { name: "CS101 Study Buddy", online: 3, initial: "CS", color: "bg-indigo-600" },
    { name: "Late Night Coders", online: 12, initial: "LN", color: "bg-slate-700" }
  ]
};
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-8 relative z-10">
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-white dark:bg-[#1E2028] px-3 py-1 rounded-full border border-gray-200 dark:border-gray-700 flex items-center gap-1 shadow-xl">
                <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Lvl</span>
                <span className="text-sm font-bold text-gray-900 dark:text-white">{user.level}</span>
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {user.tags.map(tag => (
                  <span key={tag} className="px-3 py-1 rounded-full bg-gray-100 dark:bg-[#1E2028] border border-gray-200 dark:border-gray-700/50 text-xs text-gray-700 dark:text-gray-300 font-medium">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                <Button variant="outline" className="rounded-full border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white bg-white dark:bg-[#1E2028]">
            ...
            <div className="hidden md:flex flex-col items-center text-gray-400 dark:text-gray-500 opacity-60">
              ...
              <span className="text-[10px] font-bold tracking-widest uppercase">Massachusetts Inst.</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          <div className="bg-white dark:bg-[#0F1117] p-6 rounded-2xl border border-gray-200 dark:border-gray-800/50 flex items-center justify-between group hover:border-orange-500/30 dark:hover:border-orange-900/30 transition-colors shadow-sm">
            <div>
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Study Streak</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-gray-900 dark:text-white">{user.stats.streak} Days</span>
          ...
          <div className="bg-white dark:bg-[#0F1117] p-6 rounded-2xl border border-gray-200 dark:border-gray-800/50 flex items-center justify-between group hover:border-blue-500/30 dark:hover:border-purple-900/30 transition-colors relative overflow-hidden shadow-sm">
            <div className="relative z-10">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Academic XP</p>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-gray-900 dark:text-white">{user.stats.xp.toLocaleString()}</span>
          ...
          <div className="bg-white dark:bg-[#0F1117] p-6 rounded-2xl border border-gray-200 dark:border-gray-800/50 flex items-center justify-between group hover:border-green-500/30 dark:hover:border-green-900/30 transition-colors shadow-sm">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-gray-900 dark:text-white">{user.stats.rank}</span>
                <span className="text-sm font-medium text-green-600 dark:text-green-500">{user.stats.rankNum}</span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {user.badges.map((badge, i) => (
                  <div key={i} className="bg-white dark:bg-[#0F1117] p-5 rounded-2xl border border-gray-200 dark:border-gray-800/50 hover:border-gray-300 dark:hover:border-gray-700 transition-colors flex gap-4 shadow-sm">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${badge.color}`}>
                      <badge.icon className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="font-bold text-gray-900 dark:text-gray-200 text-sm">{badge.name}</h3>
                      <p className="text-xs text-gray-500 mt-1 leading-relaxed">{badge.desc}</p>
                      <p className="text-[10px] text-blue-600 dark:text-blue-400 mt-2 font-medium">{badge.earned}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {user.courses.map((course) => (
                  <div key={course.id} className="bg-white dark:bg-[#0F1117] p-5 rounded-2xl border border-gray-200 dark:border-gray-800/50 hover:border-gray-300 dark:hover:border-gray-700 transition-colors shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button className="text-gray-400 hover:text-gray-900 dark:hover:text-white">•••</button>
                    </div>
                    <h3 className="font-bold text-gray-900 dark:text-white mb-1">{course.name}</h3>
                    <p className="text-xs text-gray-500 mb-4">Next: {course.nextTask}</p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-bold text-gray-700 dark:text-white">{course.progress}%</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6">
                  ...
                  <div className="space-y-6">
                    {user.activity.map((act, i) => (
                      <div key={i} className="relative flex gap-4 group">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-4 border-white dark:border-[#0F1117] z-10 ${act.color}`}>
                          <act.icon className="w-3.5 h-3.5" />
                        </div>
                        <div className="pt-1">
                          <h4 className="text-sm font-medium text-gray-900 dark:text-gray-200 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{act.title}</h4>
                          <p className="text-xs text-gray-500 mt-0.5">{act.meta}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {/* Active Study Groups */}
            <div>
              <h2 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-4">Active Study Groups</h2>
              <div className="bg-white dark:bg-[#0F1117] rounded-3xl border border-gray-200 dark:border-gray-800/50 overflow-hidden shadow-sm">
                {user.groups.map((group, i) => (
                  <button key={i} className="w-full flex items-center gap-4 p-4 hover:bg-gray-50 dark:hover:bg-[#1E2028] transition-colors border-b border-gray-100 dark:border-gray-800 last:border-0 text-left">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0 ${group.color}`}>
                      {group.initial}
                    </div>
                    <div className="flex-1">
                      <h4 className="text-sm font-bold text-gray-900 dark:text-gray-200">{group.name}</h4>
                      <p className="text-[10px] text-gray-500">{group.online} members online now</p>
                    </div>
