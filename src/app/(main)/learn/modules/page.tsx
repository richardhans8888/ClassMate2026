 ...
 export default function MyModulesPage() {
   ...
   const enrolledRaw = typeof window !== "undefined" ? localStorage.getItem("enrolledModules") : null;
   const enrolled = (() => {
     try {
   ...
   const modules = Array.from(new Set([...defaultModules, ...enrolled])).map((slug: string) => {
     const safe = (() => {
       try {
     ...
     <div className="min-h-screen bg-gray-50 text-gray-900 dark:bg-[#0A0F1F] dark:text-white">
       <div className="max-w-6xl mx-auto px-6 py-8">
         <div className="flex items-center justify-between">
           ...
           <Link href="/tutors">
             <Button className="bg-indigo-600 hover:bg-indigo-500">Find a Tutor</Button>
           </Link>
         </div>
         ...
         {modules.length === 0 ? (
           <div className="mt-10 rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-[#0F1117] p-8 text-center">
             ...
             <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Enroll in a module to see it here.</p>
             <div className="mt-4">
               <Link href="/tutors">
                 <Button variant="outline" className="border-gray-200 dark:border-gray-700">Explore Tutors</Button>
               </Link>
             </div>
           </div>
         ) : (
