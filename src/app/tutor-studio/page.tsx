      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-[#15181E] rounded-2xl p-6 border border-gray-800/50 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="bg-purple-500/20 text-purple-300 text-xs px-2 py-1 rounded border border-purple-500/30">New Insight</span>
            </div>
            <h3 className="text-xl font-bold mb-2">Physics Syllabus Update</h3>
            <p className="text-sm text-gray-400 leading-relaxed mb-6">
              Trending research in Quantum Mechanics suggests adding a module on "Entanglement Basics". 85% of similar premium courses have updated their curriculum this week.
            </p>
        ...
        <div className="bg-[#15181E] rounded-2xl p-6 border border-gray-800/50 flex flex-col justify-between">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xs text-green-500 font-medium bg-green-500/10 px-2 py-1 rounded">+12% vs last month</span>
          </div>
          <div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                { name: "Sarah Jenkins", course: "Advanced Calculus & Linear Algebra", time: "Submitted 2 hours ago", img: "https://api.dicebear.com/7.x/avataaars/svg?seed=Sarah", new: true },
                { name: "Marcus Chen", course: "Introduction to Python Programming", time: "Submitted 5 hours ago", img: "https://api.dicebear.com/7.x/avataaars/svg?seed=Marcus", new: false }
              ].map((req, i) => (
                <div key={i} className="bg-[#15181E] p-4 rounded-xl border border-gray-800/50 flex items-center justify-between group hover:border-gray-700 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="relative">
                      <img src={req.img} alt={req.name} className="w-12 h-12 rounded-full bg-gray-700" />
                      {req.new && <div className="absolute bottom-0 right-0 w-3 h-3 bg-yellow-500 border-2 border-[#15181E] rounded-full"></div>}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-bold text-white">{req.name}</h4>
                        {req.new && <span className="text-[10px] font-bold bg-teal-500/20 text-teal-400 px-1.5 py-0.5 rounded">NEW</span>}
                      </div>
                      <p className="text-sm text-gray-400">Requesting: <span className="text-gray-300">{req.course}</span></p>
                      <p className="text-xs text-gray-600 mt-1">{req.time}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="ghost" className="text-gray-400 hover:text-white hover:bg-white/10 border border-gray-700">Decline</Button>
                    <Button className="bg-teal-500 hover:bg-teal-600 text-black font-bold">Accept</Button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-[#15181E] rounded-2xl p-5 border border-gray-800/50 relative overflow-hidden group hover:border-gray-700 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-[10px] font-bold bg-purple-500/20 text-purple-400 px-2 py-1 rounded uppercase">Math</span>
                ...
                <p className="text-xs text-gray-500 mb-4">Mon, Wed • 10:00 AM</p>
                ...
                <div className="flex items-center -space-x-2 mt-4 pt-4 border-t border-gray-800/50">
                  {[1,2,3].map(i => (
                    <div key={i} className="w-6 h-6 rounded-full border-2 border-[#15181E] bg-gray-700 overflow-hidden">
              ...
              <div className="bg-[#15181E] rounded-2xl p-5 border border-gray-800/50 relative overflow-hidden group hover:border-gray-700 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-[10px] font-bold bg-teal-500/20 text-teal-400 px-2 py-1 rounded uppercase">Science</span>
                  <button className="text-gray-500 hover:text-white"><MoreVertical className="w-4 h-4" /></button>
                </div>
                <h3 className="font-bold text-lg mb-1">Physics 101</h3>
                <p className="text-xs text-gray-500 mb-4">Tue, Thu • 02:00 PM</p>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-[#15181E] rounded-2xl border border-gray-800/50 p-4">
            ...
            <div className="space-y-1">
              {[
                { name: "Emma W.", subject: "Calculus", streak: 12, time: "Tomorrow", subTime: "10:00 AM", img: "Emma" },
                { name: "James L.", subject: "Physics", streak: 0, time: "Today", subTime: "4:30 PM", active: true, img: "James" },
                { name: "Sophie R.", subject: "Chemistry", streak: 5, time: "Sep 24", subTime: "11:00 AM", img: "Sophie" },
                { name: "Michael T.", subject: "Geometry", streak: 8, time: "Sep 25", subTime: "09:00 AM", img: "Michael" }
              ].map((student, i) => (
                <div key={i} className="grid grid-cols-3 items-center p-2 rounded-xl hover:bg-[#1A1F26] transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gray-700 overflow-hidden">
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${student.img}`} alt={student.name} />
                    </div>
                    <div>
                      <p className="font-bold text-sm text-white">{student.name}</p>
                      <p className="text-[10px] text-gray-500">{student.subject}</p>
                    </div>
                  </div>
                  <div className="flex justify-center">
                    <div className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-full ${student.streak > 0 ? "bg-orange-500/10 text-orange-500" : "bg-gray-800 text-gray-500"}`}>
                      <Flame className={`w-3 h-3 ${student.streak > 0 ? "fill-orange-500" : ""}`} />
                      {student.streak}
                    </div>
                  </div>
                  <div className="text-right">
                    <p className={`text-xs font-bold ${student.active ? "text-teal-400" : "text-gray-300"}`}>{student.time}</p>
                    <p className="text-[10px] text-gray-500">{student.subTime}</p>
                  </div>
                </div>
              ))}
            </div>
